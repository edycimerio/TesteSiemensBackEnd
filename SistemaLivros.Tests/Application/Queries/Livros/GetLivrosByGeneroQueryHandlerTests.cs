...
namespace SistemaLivros.Tests.Application.Queries.Livros
{
    public class GetLivrosByGeneroQueryHandlerTests
    {
        ...
        public async Task RetornaLivrosPorGenero()
        {
            ...
            {
                new LivroDto { Id = 1, Titulo = "Cem Anos de Solidão", Ano = 1967, AutorId = 1, GeneroId = generoId },
                new LivroDto { Id = 2, Titulo = "O Amor nos Tempos do Cólera", Ano = 1985, AutorId = 1, GeneroId = generoId }
            };
            ...
            Assert.All(resultList, livro => Assert.Equal(generoId, livro.GeneroId));
